...
We have a function that can help use select which variables to remove.  `filter_collinear()` returns a listing of variables it suggests we keep. It attaches to the return value an attribute (like a post-it note stuck on a box) that lists the complementary variables that it suggests we drop.

```{r filter_collinear}
keep = filter_collinear(aug, cutoff = 0.7)
keep
```

Oh, that seems to suggest taking away some key variables leaving us just 4 variables 3 of which are related to the seafloor. Of course, we can decide to ignore this advice, and pick which ever ones we want including keeping them all.
...
```{r make_config}
cfg = list(
  version = "g008",               # g for greedy! 008 for August!
...
write_configuration(cfg)            # there's also a function `read_configutation()`
```

Use the `Files` pane to navigate to your personal data directory.  Open the `g008.yaml` file - this is what you configuration looks like in YAML.  Fortunately we don't mess with these much.
