...
```{r load_covar}
cfg = read_configuration(version = "g_Oct")
...
plot(nowcast['.pred_presence'], main = "Nowcast October", 
...
```{r plot_forecast}
coast = read_coastline()
plot(forecast_2075['.pred_presence'], main = "RCP85 2075 October", 
     axes = TRUE, breaks = seq(0, 1, by = 0.1), reset = FALSE)
plot(coast, col = "orange", lwd = 2, add = TRUE)
```
...
for (month in month.abb){
  ...
  iterMon = month
  versionname = sprintf("%s_%s", gsub(" ", "_",method), gsub(" ", "_", iterMon))
...
# write individual arrays?
write_prediction(nowcast, file = file.path(path,sprintf("%s_%s_RCP85_2020.tif",gsub(" ", "_", method), gsub(" ", "_", iterMon))))
write_prediction(forecast_2055, file = file.path(path, sprintf("%s_%s_RCP85_2055.tif", gsub(" ", "_",method),gsub(" ", "_", iterMon))))
write_prediction(forecast_2075, file = file.path(path, sprintf("%s_%s_RCP85_2075.tif",gsub(" ", "_",method), gsub(" ", "_", iterMon))))

# or write them together in a "multi-layer" file?
write_prediction(rcp85, file = file.path(path, sprintf("%s_%s_RCP85_all.tif", gsub(" ", "_", method), gsub(" ", "_", iterMon))))
}
}



#repeat the same thing for RCP45
methods = list("g","c")
for (method in methods){
for (month in month.abb){
...
# write individual arrays?
write_prediction(nowcast, file = file.path(path,sprintf("%s_%s_RCP45_2020.tif",gsub(" ", "_", method), gsub(" ", "_", iterMon))))
write_prediction(forecast_2055, file = file.path(path, sprintf("%s_%s_RCP45_2055.tif", gsub(" ", "_",method),gsub(" ", "_", iterMon))))
write_prediction(forecast_2075, file = file.path(path, sprintf("%s_%s_RCP45_2075.tif",gsub(" ", "_",method), gsub(" ", "_", iterMon))))

# or write them together in a "multi-layer" file?
write_prediction(rcp85, file = file.path(path, sprintf("%s_%s_RCP45_all.tif", gsub(" ", "_", method), gsub(" ", "_", iterMon))))
}
}
